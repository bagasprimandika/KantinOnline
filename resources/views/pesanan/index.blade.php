...
@section('content')
                                ...
                                <tr class="text-center">
                                    ...
                                    @elseif ($row->status == 2)
                                        <td> <span class="badge badge-danger">Konfirmasi Pembayaran</span>
                                            @if (auth()->user()->role == 1)
                                                <a href="javascript:;" data-image="{{ $row->bukti_bayar }}"
                                                    data-id="{{ $row->order_no }}" data-toggle="modal"
                                                    data-target="#imgPreview"
                                                    class="btn btn-sm btn-style rounded-circle   modal-image">
                                    ...
                                    <td>
                                        @if (auth()->user()->role == 1)
                                            ...
                                            @if ($row->status == 2)
                                                <form action="{{ route('konfirmasi_pembayaran', $row->no_pesanan) }}"
                                                    method="POST" class="d-inline">
                                                    @method('PUT')
                                                    @csrf
                                                    <button class="btn btn-primary btn-sm tombol-status"><i
                                        ...
                                        @else
                                            ...
                                            @if ($row->status == 3)
                                                <a href="#" data-id="{{ $row->id }}"
                                                    data-invoice="{{ $row->no_pesanan }}" data-toggle="modal"
                                                    data-target="#invoice" class="btn btn-sm btn-primary modalInvoice"> <i
                                                        class="fas fa-fw fa-caravan"></i></a>
                                            @endif
                                        @endif
                                    </td>
                                </tr>
    ...
    <div class="modal fade" id="invoice" data-backdrop="static" data-keyboard="false" tabindex="-1"
    ...
    <!-- Modal -->
    <div class="modal fade" id="imgPreview" data-backdrop="static" data-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Foto Bukti Pembayaran <span
                            class="judul_bukti font-weight-bolder"></span></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
